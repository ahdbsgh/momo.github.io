...
[이전 리뷰](https://momozzing.github.io/paper%20review/TripPy+SaCLog/) -> TripPy paper review
## **introduction**
...
하지만 매 턴마다 대부분의 slot은 이전 턴의 slot값을 갖고 있는것이 대부분이다. 

그래서 본 논문은 매 턴 마다 slot값을 동일하게 처리하는 메커니즘은 비효율적이고 중복된 slot값 생성에 따른 오류가 발생 한다는 문제점을 지적 

본 논문은 이러한 문제점을 해결하기 위해 현재 턴 대화를 기반으로 하는 Dual Slot Selector와 대화 기록을 기반으로 하는 Slot Value Generator 두ㄷ 단계로 구성된 DSS-DST를 제안한다. 
