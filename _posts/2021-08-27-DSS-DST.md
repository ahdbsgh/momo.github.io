...
j 번째 SLOT의 logit $\hat{y}_t^j$가 $\mathbf {logit\_sel}_t^i$와 $\mathbf {logit _ fai}_t^j$ 두 가지로 classification이 된다. 

$$H_t^j,_m = \alpha_{t}^j,_m H_{t,m}, 0 \le m < N $$
$$\hat{y}_t^j = \mathbf{softmax}(FC(H+t^j))$$

턴 t에서 j번째 slot에 대한 $Pre\_score_t^j =  \mathbf {logit\_sel}_t^i - \mathbf {logit\_fai}_t^j$를 계산한다. 

slot index의 집합을 $U_{1,t} = {\{j|Pre\_score_t^j > 0\}}$ 로 정의 
...
$\varphi_t^j$ 가 $\mathcal{V}_j$에 속하면 extract 가능한 모든 $\varphi_t^j$(임시슬롯)값의 비율을 계산하고 j번째 slot인 $Ult\_score_t^j$의 점수를 계산한다. 
...
dialog representation인 $H_t^j$ 는 $\mathcal{V}_j$의 분포를 얻기 위해 Fully connected layer 연결한다. 최대값에 해당되는 후보 slot을 새로운 임시 슬롯  $\varphi_t^j$ 로 선택하고 $\varphi_t^j$와 $None$ 사이의 분포 확률 차이를 $Ult\_score_t^j$로 계산
...
$\beta$ 는 weight, slot index set을 $U_{2,t}$ = ${\{j|Total\_score_t^j > \delta\}}$, slot index set의 size는 $J_{2,t} = |U_{2,t}|$
...
여기서 $logit\_p_t^j$는 추출 가능한 모든 임시slot값중 식 13을 통해 얻은 타겟값이고
$y\_c_{t,i}^j$는 후보 값의 확률을 나타내는 값이다. 
